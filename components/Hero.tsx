  ...
  return (
    <section className="h-screen flex items-center justify-center relative">
      ...
      <div className="mb-8 relative">
        <div className="w-48 h-48 mx-auto rounded-full border-4 border-cyan-400 overflow-hidden">
        <Image
      ...
      <h1 className="text-4xl sm:text-5xl font-bold mb-4 text-cyan-400 transition-all duration-300 hover-glow">
        Aviral Kaintura
      </h1>
      <p className="text-lg sm:text-xl mb-8 transition-all duration-300 hover-glow">
        Cybersecurity & AI Enthusiast | Computer Science Student
      </p>
      <div className="flex justify-center space-x-6 relative">
        <div className="absolute w-full h-1 bg-cyan-400 opacity-0 transition-all duration-300 ease-in-out group-hover:opacity-100 bottom-0"></div>
        {iconData.map((item, index) => (
        <div key={index} className="group relative pb-2 cursor-pointer">
          <a
          href={item.href}
          target="_blank"
          ...
          <div className="absolute hidden group-hover:block w-24 text-center top-full left-1/2 -translate-x-1/2 mt-2">
